...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[process] (matrix) at (0,-1.8)
      {Build travel-time matrix\\(ORS or Haversine fallback)};

    \node[decision] (strict) at (0,-4) {Strict order\\constraints?};

    \node[process] (applystrict) at (-4,-5.8) {Apply strict ordering\\(absolute + relative)};
    \node[process] (nn) at (4,-5.8) {Nearest Neighbour\\ordering};

    \node[process] (partition) at (0,-7.6)
      {Partition stops into\\preferred-day buckets and unassigned pool};

    % ── Phase 1: preferred-day buckets ──
    \node[process, fill=blue!15] (phase1) at (0,-9.4)
      {\textbf{Phase 1}: schedule preferred-day stops,\\order within each day bucket};

    \node[process] (initpref) at (0,-11.2)
      {Initialise day cursor at\\active-hours start + focus shift};

    \node[decision] (prefremain) at (0,-13.2) {Preferred stops\\remain in bucket?};

    \node[process] (preffit) at (0,-15.4)
      {Try fit stop on its preferred day\\(travel gap + opening hours)};

    \node[decision] (preffits) at (0,-17.8) {Stop fits\\in day?};

    \node[result] (prefplace) at (-4.5,-19.5) {Place stop;\\advance cursor};
    \node[process] (prefover) at (4.5,-19.5) {Add to overflow\\(no day spilling)};

    % ── Phase 2: unassigned stops ──
    \node[process, fill=blue!15] (phase2) at (0,-21.6)
      {\textbf{Phase 2}: schedule unassigned stops,\\order by NN or strict order};

    \node[process] (initunassign) at (0,-23.4)
      {Initialise cursor on current day at\\active-hours start + focus shift};

    \node[decision] (morestops) at (0,-25.4) {Unassigned\\stops remain?};

    \node[process] (fitstop) at (0,-27.6)
      {Try fit next stop on current day\\(travel gap + opening hours)};
    ...
    \node[process] (overflow) at (8.5,-31.8) {Add to overflow};
