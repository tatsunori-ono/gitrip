...
The Project Specification planned three testing categories: unit tests for opening-hours parsing, time arithmetic, merge functions, and CLI command parsing; integration tests for end-to-end auto-plan with mocked routing and merge workflows; and UAT via a survey on Evaluation Day. The final testing approach deviates in three justified ways. First, CLI command parsing tests were dropped because the CLI was descoped as a Could-have (C1) that was not implemented. Second, integration tests use real routing APIs with fallback verification rather than mocked routing, which provides stronger evidence of production correctness at the cost of requiring API keys during testing. Third, UAT results are presented separately in Chapter~\ref{ch:evaluation} alongside the post-demo survey analysis, rather than in this chapter.
