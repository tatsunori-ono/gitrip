...
All 64 unit tests pass. Figure~\ref{fig:unit-test-results} shows the terminal output from running the full test suite. The test runner is invoked via \texttt{npm test}, which executes \texttt{node --test 'test/*.test.js'} across all five test files. Individual test files can also be run in isolation (e.g., \texttt{node --test test/merge.test.js}) to debug specific modules.

\begin{figure}[H]
  \centering
  % \includegraphics[width=0.92\linewidth]{Chapter7/unit-test-results.png}
  \fbox{\parbox{0.92\linewidth}{\centering TODO: screenshot of terminal output after running \texttt{npm test} showing all 64 tests passing. Run from the project root.}}
  \caption{Unit test results: all 64 tests passing.}
  \label{fig:unit-test-results}
\end{figure}

\section{Functional Testing}
Functional testing verified that core workflows operate end-to-end.
Table~\ref{tab:functional_tests} summarises key cases. These workflows were
verified manually during iterative development and re-confirmed on the deployed
instance before Evaluation Day to ensure no regressions were present in the
