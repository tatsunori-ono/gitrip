...
This maps onto a conventional software life cycle~\cite{ref_sommerville}: requirements analysis, design, implementation, testing, and deployment. Rather than executing these strictly in sequence, the iterative waterfall approach revisited earlier phases when feedback demanded it (e.g., the merge engine was revised three times). This retained milestone-driven predictability while incorporating the adaptability that Sommerville identifies as a key advantage of iterative models~\cite{ref_sommerville}.
...
To sustain momentum, each week ended with an integrated build and a short review against the schedule, recording what was completed, what slipped, and why. This reduced the accumulation of partially integrated features and ensured that demonstrations for supervision and Evaluation Day rehearsal were based on the current main branch.

\section{Stakeholder Management}
The key stakeholder was the supervisor, Professor Mike Joy, with whom 12 meetings were held across both terms using the structured Cornell-inspired Obsidian notes described in Section~\ref{sec:methodology}. Each meeting followed a repeatable process: before the meeting, progress updates and questions were recorded under ``Updates from Last Meeting'' and ``Questions to Ask''; during the meeting, the supervisor's responses were noted beside each item; after the meeting, action items were summarised in the ``To-dos'' section and converted into the next week's tasks. This structured approach reduced rework by systematically organising outstanding tasks.
...
\textbf{API quota exhaustion.} OpenRouteService rate limits were hit during intensive testing, causing the planner to return errors. This was addressed with a Time-To-Live (TTL) cache, bounded concurrency (maximum four simultaneous requests), and a Haversine-based fallback so the planner degrades gracefully rather than failing entirely. The required schedule adjustment was absorbed by buffer time.

\textbf{Merge edge cases.} Early merge testing revealed that \texttt{JSON.stringify}-based comparison produced false positives when key ordering differed between branches. The merge engine was reworked to use deep structural equality and stable stop identifiers, completed before Evaluation Day and backed by 16 automated tests.

\textbf{Place search typo sensitivity.} Nominatim's exact index matching meant near-miss queries (e.g.\ ``stick n sushi'' for ``Sticks'n'Sushi'') often returned no results. This was resolved by adding Photon as a fuzzy-search fallback, called only when Nominatim returns zero results; Photon uses the same OpenStreetMap data but with Elasticsearch-based typo-tolerant indexing.

\textbf{Constraint-aware interactive editing.} Timeline drag-and-drop interactions could easily produce invalid plans (overlaps, insufficient travel time, or violations of daily active hours). A two-tier validation strategy was adopted: UI-level constraint enforcement with immediate visual feedback during edits, and server-side feasibility checks on commit.

\textbf{Increased operational work.} Deployment hardening (security headers, Content Security Policy (CSP), HTTPS, rate limiting) and public Fly.io deployment were not originally scoped but proved necessary for reliable demonstrations and remote testing. The boundary excluding generative Artificial Intelligence (AI) remained unchanged to avoid scope creep.
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{\linewidth}
    ...
    \caption{Quick explore page with high-contrast and large font enabled.}
  \end{subfigure}
